...
export default function Sidebar({ className = '' }: SidebarProps) {
  ...
  if (!isHydrated) {
    return (
      <nav className="hidden md:flex flex-col bg-slate-800 h-auto rounded-r-[18px] relative transition-all duration-500 z-10 min-w-[82px]">
        <div className="p-3">
  ...
  return (
    <>
      ...
      <nav 
        className={`
          hidden md:flex flex-col bg-slate-800 h-auto rounded-r-[18px] relative transition-all duration-500 z-10
          ${isOpen ? 'md:min-w-[220px] lg:min-w-[250px]' : 'min-w-[82px]'}
          ${className}
        `}
      >
        <div className="p-3">
          ...
          <button
            ...
            className="absolute top-[30px] -right-[10px] bg-sky-600 text-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-sky-700 transition-colors"
          >
            <i className={`
              fa-solid fa-chevron-right text-xs transition-transform duration-300
              ${isOpen ? 'rotate-180' : ''}
            `} />
          </button>
        </div>
